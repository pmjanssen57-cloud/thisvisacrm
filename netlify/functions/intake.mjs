...
export default async function intakeRequestHandler(request) {
  ...
  try {
    ...
    if (/^yes/i.test(payload.hasOtherCitizenship || '') && !payload.otherCitizenships.length) {
      return json({ error: 'Please add your other country of citizenship.' }, 400);
    }
    if (/^yes/i.test(payload.hasPartner || '') && /^yes/i.test(payload.partnerHasOtherCitizenship || '') && !payload.partnerOtherCitizenships.length) {
      return json({ error: "Please add your partner's other country of citizenship." }, 400);
    }
...
function normalisePayload(input = {}) {
  ...
  const payload = {
    ...
    hasOtherCitizenship: clean(input.hasOtherCitizenship),
    otherCitizenships: cleanCountryList(input.otherCitizenships),
    ...
    immediateNeed: clean(input.immediateNeed),
    longTermGoal: clean(input.longTermGoal),
    ...
    partnerHasOtherCitizenship: clean(input.partnerHasOtherCitizenship),
    partnerOtherCitizenships: cleanCountryList(input.partnerOtherCitizenships),
    ...
    submittedVia: formType === 'contact' ? 'THiS contact form' : clean(input.submittedVia) || 'THiS guided intake journey',
    intakeVersion: formType === 'contact' ? 'v0.12.15-contact' : clean(input.intakeVersion) || 'v0.13.26-guided',
  };
  if (!/^yes/i.test(payload.hasOtherCitizenship || '')) payload.otherCitizenships = [];
  else payload.otherCitizenships = payload.otherCitizenships.filter((country) => country !== payload.citizenship);
  if (!/^yes/i.test(payload.hasPartner || '') || !/^yes/i.test(payload.partnerHasOtherCitizenship || '')) payload.partnerOtherCitizenships = [];
  else payload.partnerOtherCitizenships = payload.partnerOtherCitizenships.filter((country) => country !== payload.partnerCitizenship);
  return payload;
}
...
function cleanCountryList(value) {
  ...
  return [...new Set(value.map((item) => clean(item)).filter(Boolean))].slice(0, 4);
}
...
function buildNewIntakeNotificationBody({ intakeId = '', payload = {}, flags = {}, createdAt = '' } = {}) {
  ...
  const lines = [
    ...
    `Physical address: ${payload.physicalAddress || 'Not recorded'}`,
...
function buildNewIntakeNotificationHtml({ intakeId = '', payload = {}, flags = {}, createdAt = '' } = {}) {
  ...
  const summaryRows = [
    ...
    ['Physical address', payload.physicalAddress],
...
function intakeSummarySections(payload = {}) {
  ...
  const sections = [
    { title: 'Your details', rows: rows(payload, ['firstName', 'lastName', 'email', 'phone', 'preferredContactMethod', 'citizenship', 'hasOtherCitizenship', 'otherCitizenships', 'dateOfBirth', 'dateOfBirthAge', 'physicalAddress', 'applicantCv']) },
    { title: 'Immigration goal', rows: rows(payload, ['targetPathway', 'immediateNeed', 'longTermGoal', 'desiredTimeframe', 'urgency', 'urgentDeadline', 'helpNeeded']) },
    ...
    { title: 'Qualifications', rows: rows(payload, ['highestQualification', 'qualificationName', 'qualificationInstitution', 'qualificationCountry', 'qualificationYearCompleted', 'qualificationStudyLength', 'taughtInEnglish', 'nzqaAssessed', 'qualificationRelatedToOccupation', 'qualificationDetails']) },
    {
      ...
      panels: [
        { title: 'Partner identity and relationship', rows: rows(payload, ['partnerFullName', 'partnerDateOfBirth', 'partnerCitizenship', 'partnerHasOtherCitizenship', 'partnerOtherCitizenships', 'partnerCurrentCountry', 'partnerVisaStatus', 'partnerNzStatus', 'livingTogether', 'relationshipStarted', 'startedLivingTogether', 'partnerIncluded', 'relationshipBackground', 'partnerCv']) },
...
function brandedInternalNotificationEmailHtml({ templateKey = '', subject = '', context = {}, bodyHtml = '' } = {}) {
  const isContact = templateKey === 'contact_form_internal_notification';
  const title = isContact ? 'New contact form enquiry' : 'New assessment form submitted';
  const tag = isContact ? 'Contact form' : 'Assessment form';
  const applicantName = context.applicantName || 'Not recorded';
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%; max-width:760px; margin:0 auto; background:#ffffff; border:1px solid #cce5dc; border-radius:18px; overflow:hidden;">
        <tr>
          <td style="padding:20px 24px; background:#003736; color:#ffffff;">
            ...
            <div style="font-family:Arial,sans-serif; font-size:22px; line-height:1.25; color:#ffffff; font-weight:700; margin-top:6px;">${escapeHtml(title)}</div>
            <div style="font-family:Arial,sans-serif; font-size:13px; line-height:1.35; color:#dff7ec; margin-top:6px;">${escapeHtml(subject || tag)}</div>
          </td>
        </tr>
        <tr>
          <td style="padding:16px 24px 4px 24px;">
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%; border-collapse:separate; border-spacing:0 8px; font-family:Arial,sans-serif;">
              <tr>
                <td style="width:34%; padding:9px 12px; background:#f4fbf8; border-radius:12px 0 0 12px; color:#53657d; font-size:12px; font-weight:700; text-transform:uppercase;">Applicant</td>
                <td style="padding:9px 12px; background:#f4fbf8; border-radius:0 12px 12px 0; color:#003736; font-size:14px; font-weight:700;">${escapeHtml(applicantName)}</td>
              </tr>
              <tr>
